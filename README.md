...
**Q: What is LightAgent?**

LightAgent is an **ultra-lightweight, open-source Agentic framework** with native **Skills** support. It enables reusable capabilities with persistent memory, tool use, and tree-of-thought reasoning.

Key features:
- **Small footprint, big potential** 🚀
- **Skills**: Modular, extensible, task-oriented capabilities
- **MCP Support**: Over stdio and SSE
- **Multi-Agent**: Self-learning agents in one step
- **Universal LLM**: OpenAI, DeepSeek, Qwen, and more
- **OpenAI-compatible APIs**: Instant drop-in with any chat interface

**Q: How does LightAgent differ from LangChain or CrewAI?**

| Framework | Key Differentiator |
|-----------|-------------------|
| **LightAgent** | **Ultra-lightweight + Native Skills + MCP** |
| LangChain | Chain-based orchestration (heavier) |
| CrewAI | Role-playing agents (more complexity) |

LightAgent is designed for **fast deployment** (spin up in 5 minutes) with **minimal footprint**.

**Q: What is Skills in LightAgent?**

**Skills** are modular, reusable capabilities introduced in v0.6.0:

- **Task-oriented**: Focused on specific tasks
- **Persistent memory**: Context retention
- **Tool integration**: External capabilities
- **Tree-of-thought**: Complex reasoning

### Getting Started

**Q: How do I install LightAgent?**
...
LightAgent runs on **any modern LLM**:
...
**Q: Does LightAgent support local models?**

Yes, via **Ollama** or any local OpenAI-compatible endpoint:
...
**Q: What built-in Skills are available?**

LightAgent provides built-in skills for common tasks. Check documentation for the latest list.
...
**Q: How does LightAgent support MCP?**

LightAgent supports MCP via:
...
**Q: What is LightSwarm?**

**LightSwarm** enables multi-agent collaboration:

- Self-learning agents in one step
- Agent coordination
- Task distribution

**Q: How do I create a LightSwarm?**

```python
from lightagent import LightSwarm
...
| **Session-level** | Task-specific memory |
...
**Q: What is Tree-of-Thought reasoning?**

Tree-of-Thought (ToT) enables **complex reasoning**:
...
**Q: What streaming APIs are supported?**

LightAgent outputs **OpenAI-compatible streaming APIs**:

```python
for chunk in agent.stream("Your question"):
    print(chunk.content)
```

Compatible with any chat interface using OpenAI format.
...
Check:
1. Skill registered correctly
2. Skill class implements `Skill` interface
3. `execute()` method returns proper format

**Q: MCP connection fails?**

Check:
1. MCP server command correct
2. MCP server running
3. Transport type (stdio/SSE) matches

**Q: Memory not persisting?**

Check:
1. `memory=True` set
2. Memory backend configured
3. Session not cleared

### Help & Resources

- **Documentation**: [sufe-aiflm-lab.github.io/LightAgent](https://sufe-aiflm-lab.github.io/LightAgent/)
- **PyPI**: [pypi.org/project/lightagent](https://pypi.org/project/lightagent/)
- **arXiv Paper**: [arxiv.org/abs/2509.09292](https://arxiv.org/abs/2509.09292)
- **GitHub**: [wanxingai/LightAgent](https://github.com/wanxingai/LightAgent)
- **Issues**: [GitHub Issues](https://github.com/wanxingai/LightAgent/issues)

---

*For more details, see the [Documentation](https://sufe-aiflm-lab.github.io/LightAgent/) and [GitHub repository](https://github.com/wanxingai/LightAgent).*